    ...
    return (
      <Layout location={this.props.location} title={title}>
        ...
        {posts.map(({ node }) => {
          ...
          return (
            <Row key={slug} style={{ marginBottom: "3em" }}>
              <Col
                xsOffset={1}
                xs={10}
                smOffset={1}
              ...
              >
                <TagSpan>
                  <PostLink to={`/${template}`}>
                    <MetaText style={{ display: "inline", fontWeight: 700 }}>
                      {template}:&nbsp;&nbsp;
                    </MetaText>
                  </PostLink>
                  {tags.map((tag, index) =>
                    index === 0 ? (
                      <PostLink to={`/tags/${kebabCase(tag)}/`}>
                        <MetaText style={{ display: "inline" }}>{tag}</MetaText>
                      </PostLink>
                    ) : (
                      <TagSpan key={index}>
                        &nbsp;&nbsp;&middot;&nbsp;&nbsp;
...
export const pageQuery = graphql`
  query allMdx($skip: Int!, $limit: Int!) {
    site {
      siteMetadata {
        title
        description
